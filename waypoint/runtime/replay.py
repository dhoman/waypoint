...
from waypoint.domain.artifact import Capability, Result
from waypoint.evidence.trace import sanitized
from waypoint.runtime.delivery import DeliveryLedger
from waypoint.runtime.interpret import check, extract, recognize
from waypoint.surfaces.protocol import Surface


class Replay:
    def __init__(self, capability: Capability, surface: Surface, inputs, trace):
        self.cap = Capability.model_validate(capability.model_dump())
        self.inputs = self.cap.validate_inputs(inputs)
        surface.validate_capability(self.cap)
        self.surface, self.trace = surface, trace
    ...
    async def observe(self):
        start = monotonic()
        self.obs = await self.surface.observe()
        kind, screen, checks = await recognize(
            self.cap.screens, self.obs, self.inputs, self.surface
        )
        self.trace.emit(
            "recognition",
            state=self.state,
    ...
    async def choose(self, state):
        ...
        for t in self.cap.transitions:
            if t.source == state and all(
                await check(t.guards, self.obs, self.inputs, self.surface)
            ):
                matches.append(t)
        if len(matches) != 1:
            raise ValueError(
                ...
                async with asyncio.timeout(t.timeout_s):
                    ...
                    if t.action.kind == "extract":
                        self.outputs = await extract(
                            self.cap.output, self.obs, self.inputs, self.surface
                        )
                    elif t.action.kind != "outcome":
                        await self.delivery.attempt(
                            t.id, lambda: self.surface.act(t.action, self.inputs)
                        )
                    dest = t.destination
