...
async def holds(predicate, observation, inputs, surface: Surface):
    p = predicate
    refs = [p.input]
    if p.target:
    ...
    if p.op in {"target_present", "target_text_input"}:
        try:
            resolved = await surface.resolve(p.target, inputs)
            return (
                resolved["visible"]
                if p.op == "target_present"
                else await surface.read(p.target, inputs) == str(inputs[p.input])
            )
        except ValueError:
            return False
...
async def check(predicates, obs, inputs, surface: Surface):
    return [await holds(p, obs, inputs, surface) for p in predicates]


async def recognize(screens, obs, inputs, surface: Surface):
    checks = {
        key: await check(screen.recognition, obs, inputs, surface)
    ...
    for key in matches:
        if not all(await check(screens[key].identity, obs, inputs, surface)):
...
async def extract(specs, observation, inputs, surface: Surface):
    output = {}
    for spec in specs:
        if spec.source == "input":
            ...
            value = await surface.read(spec.target, inputs, spec.attribute)
