...
from waypoint.domain.artifact import Observation, Target
from waypoint.evidence.trace import sanitize_manual_event
from waypoint.runtime.ownership import Ownership
from waypoint.surfaces.browser.policy import Policy, origin
...
class Browser:
    ...
    def __init__(self, url, *, headed=False, policy=None):
        ...
        self.binding = SurfaceBinding(origin(url), urlsplit(url).path or "/")
        self.ownership = Ownership()
        self.event_sink = lambda *args, **kwargs: None
        self.violation = None

    def validate_capability(self, capability):
        if capability.surface != "browser" or origin(self.url) != capability.origin:
            raise ValueError(
                "policy: artifact belongs to another surface or origin; rediscover for this site"
            )
