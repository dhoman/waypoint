...
def test_inspector_embeds_real_artifact_and_run_without_executable_ui_text(tmp_path):
    artifact = Capability.model_validate_json(
        open("evidence/web-books-discovery-5/capability.json").read()
    )
    run = tmp_path / "run"
    run.mkdir()
    ...
    assert "web-fd5451c60d" in rendered
...
async def test_inspector_selects_failed_transition_and_renders_rich_evidence(tmp_path):
    ...
    render_inspector(artifact, [Path("evidence/web-books-missing")], output)
    async with async_playwright() as pw:
        ...
        assert await page.locator("#detail-title").inner_text() == "Transition t0"
        assert (
            "ambiguous or missing target" in await page.locator("#detail").inner_text()
        )
